...
Note: MacBook Air M1 with 16GB RAM was used for testing.

---

# Solution Details
This section will provide details about the two proposed solutions.
...
originally anticipated. Due to the time limitations, I chose to implement this alternative solution 
...
and the slice of bytes representing the bitstore ($2^{29}$ bytes=512MB, given 1MB=1,048,576 bytes).
...
in the binary representation of the hashed value. The intuition is that it’s unlikely to observe a long pattern of trailing zeros unless you’ve seen many different numbers.
...
This relies on multiple assumptions regarding the distributions of the hash values, their distinction, hash space, and the distinctiveness of the dataset itself.

The mathematical explanation of this algorithm is beyond the scope of this project but such information can be found in the
paper provided by the creators of the algorithm [2].
...
The authors of the original algorithm (see [2]) provide the estimations of error % based on the number of hash functions used as shown below:

<img src="data/fm_expected_accuracies.png" alt="FM Expected Accuracy" width="350">
...
was comparable for the 16 hash and 128 hash condition while offering a roughly 5 times faster execution, 16 instances of the xxhash function (each with
a different seeds) was selected for the final test.
